...
export default function PackagesPrices() {
    ...
    const pricing = packageTypes[activePackageType].data;
            ...
            <div className="relative z-1 max-w-7xl mx-auto px-7 text-center md:my-10">
                ...
                <div className="grid grid-cols-2 md:flex md:flex-wrap md:justify-center gap-4 mb-8">
                    {Object.entries(packageTypes).map(([key, config]) => (
                        <button
                            key={key}
                            onClick={() => handlePackageTypeChange(key)}
                            className={`flex items-center gap-1 pr-3 pt-1 rounded-2xl border-2 font-semibold transition-all duration-300 text-sm md:text-base shadow-md hover:shadow-lg ${activePackageType === key
                                    ? 'bg-[#2072CC] text-white border-[#2072CC] shadow-lg transform scale-105'
                                    : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50 hover:border-[#2072CC] hover:text-[#2072CC]'
                                }`}
                        >
                            <div className={`flex-shrink-0 transition-all duration-300 ${activePackageType === key ? 'filter brightness-0 invert' : 'opacity-70'
                                }`}>
                                <Image
                                    src="/images/shared/package-price-icon.svg"
                                    width={50}
                                    height={50}
                                    alt="Package Button Icon"
                                />
                            </div>
                            {config.label}
                        </button>
