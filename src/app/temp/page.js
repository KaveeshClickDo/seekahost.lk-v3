      ...
      <div className="container mx-auto px-4">
        ...
        <div className="relative">
          ...
          {timelineData.map((item, index) => (
            <div 
              key={index} 
              className={`mb-8 flex items-center ${index % 2 === 0 ? 'justify-start' : 'justify-end'} relative`}
            >
              {/* Timeline dot */}
              <div className="absolute left-1/2 transform -translate-x-1/2 w-6 h-6 rounded-full bg-blue-500 border-4 border-gray-200 z-10"></div>
              
              {/* Content box */}
              <div 
                className={`w-5/12 ${index % 2 === 0 ? 'pr-1' : 'pl-1'}`}
              >
                <div className="relative bg-white p-6 rounded-lg shadow-md border border-gray-200">
                  {/* Blue vertical line */}
                  <div className={`absolute top-0 ${index % 2 === 0 ? '-left-2' : '-left-2'} w-5 h-full bg-blue-500 rounded-l-2xl`}></div>
